...
fn recover_from_file() -> Result<()> {
    ...
    println!(
        "Recovery file loaded. UUID: {} Timestamp: {}",
        recovery.uuid, recovery.timestamp
    );
    ...
    println!(
        "If you have a backup of your previous memory file, enter its path to restore it now."
    );
    ...
    loop {
        ...
        match choice.trim() {
            ...
            "3" => {
                ...
                save_memory(
                    &mut memory,
                    master_key.as_ref().ok_or(anyhow!("Run onboarding first"))?,
                    &key,
                    &value,
                    &tags,
                )?;
...
fn onboard() -> Result<()> {
    ...
    std::fs::OpenOptions::new()
        ...
        .write_all(audit_entry.as_bytes())?;
    println!(
        "Onboarding complete. Recovery file created: {}",
        RECOVERY_FILE
    );
    Ok(())
}
...
fn search_notes(memory: &Memory, keyword: &str) -> Result<()> {
    ...
    for (k, note) in &memory.items {
        if k.contains(keyword)
            || note.value.contains(keyword)
            || note.tags.iter().any(|t| t.contains(keyword))
        {
...
fn switch_user() -> Result<()> {
    ...
    if !std::path::Path::new(&user_memory_file).exists() {
        ...
        fs::write(
            &user_memory_file,
            encrypt_memory(&Memory::default(), &load_master_key()?)?,
        )?;
    }
